...
```{r}
#master <- readxl::read_excel(path="D:\\Libraries 1\\Documents\\QMSS\\Data For Good\\data\\current_master_2020-Oct-7.xlsx")
#head(master)
...
#clean the master col names 
```{r}
#colnames(master) <- c("unique_id", "source", "census_year", "state_province", "county", "place",  "household_joint_id", "joint_id_for_matched_records", "last_name", "first_name", "calculated_birth_year", "age", "sex", "color_race_or_ethnicity", "lat", "long", "address", "marital", "ward", "roll_or_sheet_number", "profession", "notable", "street", "place_of_birth", "religion", "note_these_only_apply_to_narrative_answers", "living_with_male_family", "living_with_female_family", "living_with_male_nonfamily", "living_with_female_nonfamily", "cannot_read", "cannot_write", "sick", "relation_to_head_of_household", "year_of_immigration_to_canada_if_an_immigrant", "date_of_death", "cause_of_death", "rank_military", "enlistment_date", "enlistment_place", "date_mustered_out", "year_of_this_record", "last_name_match", "first_name_match", "census_year_match", "total_of_matches")
#head(master)
```
...
#preprocess <- readxl::read_excel(path="D:\\Libraries 1\\Documents\\QMSS\\Data For Good\\DataPreprocessingtoMaster\\preprocess.xlsx", sheet=2)
#head(preprocess)
...
preprocess %>% 
  select("Source", "State", "County", "County", "City", "Household Joint ID", "Joint ID for Matched Records", "Surname", "Given Name", "Estimated Birth Year", "Age", "Gender", "Race", "Street Address", "Marital Status", "Occupation", "Dwelling Number", "Birthplace", "Able to Read and Write", "Veteran", "War") 



preprocess$Source <- gsub(' census', '', preprocess$Source)
head(preprocess)



preprocess <- preprocess %>% 
  select("unique_id", "Source", "State", "County", "County", "City", "Household Joint ID", "Joint ID for Matched Records", "Surname", "Given Name", "Estimated Birth Year", "Age", "Gender", "Race", "Street Address", "Marital Status", "Occupation", "Dwelling Number", "Birthplace", "Able to Read and Write", "Veteran", "War") 
preprocess

colnames(preprocess) <- c("census_year", "state_province", "county", "place", "household_join_id", "joint_id_for_matched_records", "last_name", "first_name", "calculated_birth_year", "age", "sex", "color_race_or_ethnicity", "address", "marital", "profession", "street", "place_of_birth", "cannot_read", "rank_military", "enlistment_place" )
head(preprocess)
...
#final_join <- data.frame(lapply(final_join, as.character), stringsAsFactors = FALSE)
#export to csv
#write.csv(x=final_join, file="preprocessfinal12_08_2020.csv")
